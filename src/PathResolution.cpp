...
value_t PathResolution::createPaths(Environment* envAtResolution) {
    return this->createPaths(this->path, envAtResolution);
}
...
value_t PathResolution::createPaths(const Lvalue& lvalue, Environment* envAtResolution) {
    ::activeCallStack.push_back(lvalue);
    defer {safe_pop_back(::activeCallStack);};
    return std::visit(overload{
        [this, envAtResolution](auto* lvaluePtr){
            return this->createPaths(*lvaluePtr, envAtResolution);
        },
        [this, envAtResolution](Symbol* symbol){
            return this->evaluateValue(symbol, envAtResolution);
        }
    }, lvalue.variant);
}

// see PathResolution::evaluateValue(FieldAccess)
...
// see PathResolution::evaluateValue(Subscript)
value_t PathResolution::createPaths(const Subscript& subscript, Environment* envAtResolution) {
    extern value_t SENTINEL_NEW_MAP; // defined in src/interpret.cpp //
    defer {this->nthSubscript += 1;};
