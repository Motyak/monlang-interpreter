...
static value_t divFloat(Float firstArgValue, const std::vector<FlattenArg>& args);

extern uint64_t builtin_lambda_id; // defined in src/interpret.cpp

const value_t builtin::op::div __attribute__((init_priority(3000))) = new prim_value_t{prim_value_t::Lambda{
    builtin_lambda_id++,
    IntConst::TWO,
    [](const std::vector<FlattenArg>& args) -> value_t {
        ...
        return std::visit(overload{
            [&otherArgs](Byte byte) -> value_t {return divFloat(byte, otherArgs);},
            [&otherArgs](Int int_) -> value_t {return divFloat(int_, otherArgs);},
            [&otherArgs](Float float_) -> value_t {return divFloat(float_, otherArgs);},
