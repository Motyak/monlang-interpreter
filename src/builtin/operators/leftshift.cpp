...
static value_t leftshiftByte(Byte firstArgValue, const std::vector<FlattenArg>& args);
static value_t leftshiftInt(Int firstArgValue, const std::vector<FlattenArg>& args);

extern uint64_t builtin_lambda_id; // defined in src/interpret.cpp

const value_t builtin::op::leftshift __attribute__((init_priority(3000))) = new prim_value_t{prim_value_t::Lambda{
    builtin_lambda_id++,
    IntConst::TWO,
    [](const std::vector<FlattenArg>& args) -> value_t {
        ...
        return std::visit(overload{
            [&otherArgs](Byte byte) -> value_t {return leftshiftByte(byte, otherArgs);},
            [&otherArgs](Int int_) -> value_t {return leftshiftInt(int_, otherArgs);},
